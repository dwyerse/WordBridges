...
public class manager : MonoBehaviour {
    ...
    void levelComplete()
    {
        ...
        cS.transform.localScale = new Vector3(3,4,1);
    }
        ...
        for(int i=0;i< str.GetLength(0); i++)
        {
            for (int j = 0; j < str.GetLength(1); j++)
            {
                if (str[i, j] != null)
                {
                    ...
                    Vector3 vec = new Vector3(-0.6f+(float)(j*0.8),2f-(float)(i*0.8),0);
    ...
    void createLetters(string[,] str)
    {
        ...
        for (int i = 0; i < str.GetLength(0); i++)
        {
            for (int j = 0; j < str.GetLength(1); j++)
            {
                ...
                {
                    GameObject con = new GameObject();
                    Vector3 vec = new Vector3(-0.6f + (float)(resetOff * 0.8), -1.5f + rowOff , 0);
                    resetOff++;
                    ...
                    con.AddComponent<SpriteRenderer>();
                    con.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("Images/letterbox");
                    con.GetComponent<SpriteRenderer>().color = Color.cyan;
        ...
        for (int a=0;a<letters.Length;a++)
        {
            ...
            Vector3 vec = new Vector3(-0.6f + (float)(resetOff * 0.8), -1.5f + rowOff, -1);
            resetOff++;
            if (a % 3 == 0 && a != 0)
            {
                rowOff--;
            ...
            letter.GetComponent<TextMesh>().characterSize = 0.02f;
            letter.GetComponent<TextMesh>().fontSize = 162;
